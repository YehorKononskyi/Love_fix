...
// v13.0 — чтение конфигурации из ./data/*.json (GitHub Pages friendly, без module)
const $  = (s)=> document.querySelector(s);
const wait = (ms)=> new Promise(r=> setTimeout(r, ms));

// Глобалы состояния
let CFG=null, CARDS=[], CATS=null, SELECTED=new Set(), ORDER=[], IDX=-1, HIST=[], ORDER_MODE="shuffle", BUSY=false;

// Загрузка JSON'ов
async function loadJSON(url){
  const v = (CFG&&CFG.version) ? `?v=${encodeURIComponent(CFG.version)}` : `?v=${Date.now()}`;
  const full = url + v;
  const res = await fetch(full, {cache:"no-store"});
  if(!res.ok) throw new Error(`Fetch failed ${res.status} for ${full}`);
  return res.json();
}
...
async function boot(){
  try{
    ...
    CFG = {
      ...
      categories: [
        {"id":"cat1","name":"Тест 1","fill":"#DDF5D6","stroke":"#3E83A4","text":"#39434C"},
        {"id":"cat2","name":"Тест 2","fill":"#D9EFF6","stroke":"#3E83A4","text":"#3E4A54"},
        {"id":"cat3","name":"Тест 3","fill":"#F3F3C8","stroke":"#9F8F3E","text":"#5A5A44"}
      ]
    };
  }
  ...
  try{
    ...
    // Фолбэк
    CARDS = [
      {"id":"c1_q1","cat":"cat1","text":"[Тест 1] Вопрос 1"},
      {"id":"c1_q2","cat":"cat1","text":"[Тест 1] Вопрос 2"},
      {"id":"c1_q3","cat":"cat1","text":"[Тест 1] Вопрос 3"},
      {"id":"c1_q4","cat":"cat1","text":"[Тест 1] Вопрос 4"},
      {"id":"c1_q5","cat":"cat1","text":"[Тест 1] Вопрос 5"},

      {"id":"c2_q1","cat":"cat2","text":"[Тест 2] Вопрос 1"},
      {"id":"c2_q2","cat":"cat2","text":"[Тест 2] Вопрос 2"},
      {"id":"c2_q3","cat":"cat2","text":"[Тест 2] Вопрос 3"},
      {"id":"c2_q4","cat":"cat2","text":"[Тест 2] Вопрос 4"},
      {"id":"c2_q5","cat":"cat2","text":"[Тест 2] Вопрос 5"},

      {"id":"c3_q1","cat":"cat3","text":"[Тест 3] Вопрос 1"},
      {"id":"c3_q2","cat":"cat3","text":"[Тест 3] Вопрос 2"},
      {"id":"c3_q3","cat":"cat3","text":"[Тест 3] Вопрос 3"},
      {"id":"c3_q4","cat":"cat3","text":"[Тест 3] Вопрос 4"},
      {"id":"c3_q5","cat":"cat3","text":"[Тест 3] Вопрос 5"}
    ];
  }

  // Применить прозрачность фоновой накладки
  document.body.style.setProperty('--bgov-o', '0'); // старт с 0, потом поднимем до CFG.bgOverlayOpacity в конце онбординга

  CATS = byCat(CFG.categories, CARDS);
  setSelected(Object.keys(CATS));
  setOrderMode(CFG.defaultOrder === "sequential" ? "sequential" : "shuffle");
  updateProgress();
  bind();
  runIntro();
...
// Утилиты
function byCat(categories, cards){
  const m = {};
  for(const c of categories) m[c.id] = {...c, cards:[]};
  for(const x of cards) if(m[x.cat]) m[x.cat].cards.push(x);
  return m;
}
...
// Порядок/прогресс
function buildOrder(){ const arr = CARDS.filter(x=> SELECTED.has(x.cat)); ORDER = (ORDER_MODE==="shuffle") ? shuffle(arr.map(x=>x.id)) : arr.map(x=>x.id); IDX=-1; updateProgress(); }
function cur(){ if(ORDER.length===0) return null; const id=ORDER[IDX]; return CARDS.find(c=>c.id===id)||null; }
...
async function transitionTo(card, overlapColors=true){
  ...
  BUSY = true; inner.classList.add("fade-out"); if(card && overlapColors) applyPalette(card.cat); await wait(220);
  ...
  inner.style.transform = "translateY(-18px)"; void inner.offsetHeight; inner.classList.remove("fade-out"); await wait(220); inner.style.transform = "";
...
function openDeckDialog(){
  ...
  list.addEventListener("change", (e)=>{
    if(e.target.id==="deck_all"){ const on = e.target.checked; list.querySelectorAll(".deckChk").forEach(c=> c.checked = on); }
    else if(e.target.classList.contains("deckChk")){ const allOn = Array.from(list.querySelectorAll(".deckChk")).every(c=> c.checked); $("#deck_all").checked = allOn; }
  }, {once:false});
...
// Онбординг (+плавное появление фоновой накладки)
function runIntro(){
  ...
  if(skip){
    intro.classList.add("hidden"); $("#app").removeAttribute("aria-hidden");
    document.body.style.setProperty('--bgov-o', String(CFG.bgOverlayOpacity ?? 0.3)); return;
  }
  ...
  function endIntro(){
    ...
    setTimeout(()=>{
      ...
      requestAnimationFrame(()=>{
        ...
        document.body.style.setProperty('--bgov-o', String(CFG.bgOverlayOpacity ?? 0.3));
        setTimeout(()=>{
